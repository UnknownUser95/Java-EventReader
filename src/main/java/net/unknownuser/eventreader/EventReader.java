...
public class EventReader implements Runnable {
    ...
    public static Optional<EventReader> runAtEventNumber(
        int eventNumber,
        boolean filterSeparators,
        boolean filterMiscellaneous
    ) {
    ...
    private static Optional<EventReader> validate(
        String path,
        boolean filterSeparators,
        boolean filterMiscellaneous
    ) {
    ...
    /**
     * Gets all available mice from {@code /dev/input/by-id}. Mouse events end with {@code -event
     * -mouse}.
     *
    ...
    private static List<String> filterWithinID(String filter) {
        File[] files = new File("/dev/input/by-id/").listFiles(file -> file
            .getName()
            .endsWith(filter));
        
        if (files == null) {
            System.out.println("No inputs exist. Cannot continue.");
            System.exit(1);
        }
    ...
    /**
     * Discards this EventReader. The reader thread cannot be stopped on its own and needs one
     * new input event to be
     * properly stopped.<br>
        ...
        try {
            while (!discarded) {
                ...
                if ((numBytes = inputStream.read(bytes, 0, bytes.length)) != InputEvent.BYTES) {
                    System.out.printf(
                        "Wrong amount of bytes read. Expected %d, got %d%n",
                        InputEvent.BYTES,
                        numBytes
                    );
                    break;
